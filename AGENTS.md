...
> ✅ **原定任务已全部完成（M1–M6）**：上游 Lidless 已改造为 NightCat（四档递进、
> 中英双语、档位锁定、防忘关安全网、Sparkle 自动更新）。以下保留原始任务说明，
> 供接手者了解背景；文末「交付与验证习惯」仍然有效。

## 原始任务

把 fork 过来的上游 Lidless 改造成 **NightCat**：把单一「合盖不睡」开关扩展为**四档递进**的保持唤醒工具，中文界面，加上档位锁定与防忘关的安全网。功能规格以 `docs/SPEC.md` 为准，任务顺序见 `docs/PLAN.md`。

上游代码是**可用的基线**，不是要重写的东西。提权 helper、XPC、看门狗、签名都是现成且经过验证的，沿用即可。
...
| 项 | 要求 |
|---|---|
| Xcode | 15+（String Catalog） |
| XcodeGen | 必需 |
| 签名证书 | 运行 App 与安装 Helper 需要 |
